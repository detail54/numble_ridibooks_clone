...
// interface
import { ISearchBox } from '../interface/interface'
// style
import { SearchBoxSC } from './SearchBox.styles'

const SearchInput: React.FC<ISearchBox> = (props) => {
  const {
    searchText,
    placeholder,
    onChange,
  ...
  const cancleButton = openCancelButton ? (
    <SearchBoxSC.CancleButton onClick={(event) => onRemove && onRemove(event)}>
      <SearchBoxSC.CancleIcon viewBox='0 0 14 14' aria-label='지우기'>
        <path
          fill='currentColor'
        ...
        />
      </SearchBoxSC.CancleIcon>
    </SearchBoxSC.CancleButton>
  ) : (
    <></>
  )

  return (
    <SearchBoxSC.Form maxWidth={styles && styles.maxWidth}>
      <SearchBoxSC.Div>
        <SearchBoxSC.SearchIcon viewBox='0 0 24 24' aria-label='인스턴트 검색'>
          <path
            fill='currentColor'
